...
<interview_completion>
    ...
    <closing_message>
        Тепло поблагодари за уделённое время и ценные инсайты.
        Скажи, что ответы очень помогут в развитии продукта.
    </closing_message>

    <output_instruction>
        После прощального сообщения ОБЯЗАТЕЛЬНО выведи JSON-блок с результатами.
        JSON должен быть обёрнут в теги: ```custdev_result ... ```
    </output_instruction>

    <json_schema>
```custdev_result
        {{
            "custdev_result": "описание всего что сказал респондент",
        }}
```
    </json_schema>
